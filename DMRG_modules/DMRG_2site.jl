"""
 ...
 [M,E0,Eiter] = DMRG_2site (Hs,Nkeep,Nsweep [, option])
...
"""
function DMRG_2site(Hs,Nkeep,Nsweep,x...)
...
# show message
if isempty(Morth)
	println("Two-site DMRG: search for the ground state.");
else
	println("Two-site DMRG: search for the lowest-energy state orthogonal to ''Morth''.")
end
...
for itS in (1:Nsweep)
    # right -> left
   for itN in (N:-1:2)
	# merge two rank-4 tensors for the two lattice sites to a single rank-4 tensor
	Hloc = contract(Hs[itN-1],4,[3],Hs[itN],4,[1], [1,2,4,5,3,6]);
	# merge the physical legs of Hloc by reshaping
	Hloc = reshape(Hloc,(size(Hloc,1),size(Hloc,2)*size(Hloc,3),size(Hloc,4),size(Hloc,5)*size(Hloc,6)));
	
	# merge two rank-3 ket tensors to a single rank-3 tensor
	Ain = contract(M[itN-1],3,[3],M[itN],3,[1]);
	# merge the physical legs of Ain by reshaping
	Ain = reshape(Ain,(size(Ain,1),size(Ain,2)*size(Ain,3),size(Ain,4)));

       if isempty(Morth)
           # Use eigs_1site to obtain the variationally chosen ket tensor
           # Aeff and energy expectation value Eeff
           Aeff,Eeff = eigs_1site(Hlr[itN-1],Hloc,Hlr[itN+2],Ain);
       else
	   ...
	   Aorth = contract(Morth[itN-1],3,[3],Morth[itN],3,[1]);
	   # merge the physical legs of Ain by reshaping
           Aorth = reshape(Aorth,(size(Aorth,1),size(Aorth,2)*size(Aorth,3),size(Aorth,4)));

           if !isempty(Olr[itN-1])
               Aorth = contract(Olr[itN-1],2,[2],Aorth,3,[1]);
           end
           ...
           Aeff,Eeff = eigs_1site(Hlr[itN-1],Hloc,Hlr[itN+2],Ain,"Aorth",Aorth);
       end

        Eiter[N+1-itN,2*itS-1] = Eeff;

       # split the local leg by reshaping
       Aeff = reshape(Aeff,(size(Aeff,1),size(Hs[itN-1],4),size(Hs[itN],4),size(Aeff,3)));
       # update M{itN} and M{itN-1} by using Aeff, via SVD
       # decompose Aeff
        UT,ST,M[itN] = svdTr(Aeff,4,[1,2],Nkeep,[]);
	STdiag = zeros(size(ST,1),size(ST,1));
	for i in 1:size(ST,1)
		STdiag[i,i] = ST[i];
	end
       # contract UT*STdiag with M{itN}, to update M{itN}
           M[itN-1] = contract(UT,3,[3],STdiag,2,[1]);
    ...
    print_time();
    println(" | Sweep # $(2*itS-1)/$(2*Nsweep) (right -> left) : Energy = $(Eiter[N-1,2*itS-1])");
    if !isempty(Morth)
    ...
    for itN in 1:(N-1)
	# merge two rank-4 tensors for the two lattice sites to a single rank-4 tensor
	Hloc = contract(Hs[itN],4,[3],Hs[itN+1],4,[1],[1 2 4 5 3 6]);
	# merge the physical legs of Hloc by reshaping
	Hloc = reshape(Hloc,(size(Hloc,1),size(Hloc,2)*size(Hloc,3),size(Hloc,4),size(Hloc,5)*size(Hloc,6)));

	# merge two rank-3 ket tensors to a single rank-3 tensor
	Ain = contract(M[itN],3,[3],M[itN+1],3,[1]);
	# merge the physical legs of Ain by reshaping
	Ain = reshape(Ain,(size(Ain,1),size(Ain,2)*size(Ain,3),size(Ain,4)));

        if isempty(Morth)
            Aeff,Eeff = eigs_1site(Hlr[itN],Hloc,Hlr[itN+3],Ain);
        else
            # vector whose orthogonal subspace is to be considered
	    # merge two rank-3 ket tensors to a single rank-3 tensor
	    Aorth = contract(Morth[itN],3,[3],Morth[itN+1],3,[1]);
	    # merge the physical legs of Ain by reshaping
            Aorth = reshape(Aorth,(size(Aorth,1),size(Aorth,2)*size(Aorth,3),size(Aorth,4)));
            ...
            if !isempty(Olr[itN+3])
                Aorth = contract(Aorth,3,[3],Olr[itN+3],2,[2]);
            end

            Aeff,Eeff = eigs_1site(Hlr[itN],Hloc,Hlr[itN+3],Ain,"Aorth",Aorth);
        end

        Eiter[itN,2*itS] = Eeff;

	# split the local leg by reshaping
	Aeff = reshape(Aeff,(size(Aeff,1),size(Hs[itN],4),size(Hs[itN+1],4),size(Aeff,3)));
        # update M{itN} and M{itN+1} by using Aeff, via SVD
        # decompose Aeff
        M[itN],ST,VT = svdTr(Aeff,4,[1,2],Nkeep,[]);
	STdiag = zeros(size(ST,1),size(ST,1));
        for i in 1:size(ST,1)
                STdiag[i,i] = ST[i];
        end
        # contract UT*STdiag with M[itN], to update M{itN}
	M[itN+1] = contract(STdiag,2,[2],VT,3,[1]);
    ...
    # display informaiton of the sweep
    print_time();
    println(" | Sweep # $(2*itS)/$(2*Nsweep) (left -> right) : Energy = $(Eiter[N-1,2*itS])");
    if !isempty(Morth)
	println("Overlap with Morth = $(Olr[N+1])");
    end
end

E0 = Eiter[N-1,2*Nsweep]; # take the last value
toc = time() - tic;
